...
Next step is to check if the obstacle is to the right or left of the queen on that line, followed by closest element of right and left.
And finally calculate the possible ways to move by calculating the positions between queen and closest elements to right and left.
...
For checking if the obstacle lies on the right or left of the queen for each horizontal,forward and backward line, we can compare
columns of the obstacle and queen. Whereas for the vertical line, we have to use row of obstacle for comparing.

For example, suppose on a horizontal line queen is at (4,4) with n = 8. Then for all the obstacle on the left (suppose (4,1) and (4,2))
the closest one to the queen is with column=2. Hence we have used variable max_left to store the next free location i.e. max_left = 3.
Similarly for right the closest one to the queen will be min_right (you can guess why :) ).

For horizontal and vertical line we initiate the max_left and min_right to 1 and n respectively, where max_left and max_right are the
variables that hold the leftmost and rightmost obstacle free position.

For forward and backward line initiating the max_left and min_right is bit tricky.
For forward line the max_left (c_q - min((r_q - 1),(c_q - 1))), where the term (min((r_q - 1),(c_q - 1))) gives the available blocks
to the left of the queen on forward line. The terms checks which is minimum rows or columns to the left; as we can only move these many
steps or we will be out of the chess board.
'''
...
def pos_count(max_left,min_right,queens_loc):
    ...
    return (queens_loc - max_left) + (min_right - queens_loc)
...
def queensAttack(n, k, r_q, c_q, obstacle):
    ...
    #initiate the variables to hold leftmost and rightmost possible column till where queen can move
    hl_max_left, hl_min_right = 1, n    #explained on line 30
    vl_max_bot, vl_min_top = 1, n       #vertical line

    #calculate the leftmost and rightmost possible column of the inclined lines till where queen can move
    fl_max_left =  c_q - min((r_q - 1),(c_q - 1))   #this step is explained on line 32.
    fl_min_right = c_q + min((n - r_q),(n - c_q))   #forwardline
    bl_max_left = c_q - min((n - r_q),(c_q - 1))    #backwardline
    bl_min_right = c_q +  min((r_q - 1),(n - c_q))
    ...
    vl_count = pos_count(vl_max_bot,vl_min_top, r_q)     # note that as we compared the row for queen we use r_q
